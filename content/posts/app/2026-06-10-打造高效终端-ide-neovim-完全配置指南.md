...
> 2000+ 次提交，八年持续打磨。一套好的编辑器配置，不是插件堆砌，而是对工作流的深度理解。
...
`format_on_save` 针对不同文件类型做了差异化配置：
- **Go**：保存时先执行 `source.organizeImports`（代码操作），再进行格式化
- **Lua**：保存时执行 stylua 格式化，LSP fallback
- 其他语言：保存时不自动格式化，按需手动触发 `<Leader>fm`
...
2026 年 6 月，venux 的核心搜索框架从 Telescope 全面迁移到了 [folke/snacks.nvim](https://github.com/folke/snacks.nvim) 的 picker 模块。Snacks.nvim 是一个由 lazy.nvim 作者开发的集合性插件，其 picker 模块在保持了 Telescope 核心体验的同时，更加轻量且与 lazy.nvim 生态深度整合。

这次迁移带来了几个关键收益：

- **零额外依赖**：Telescope 依赖 plenary.nvim 等额外库；Snacks 直接内建在 lazy.nvim 生态中
- **更快的启动**：Snacks 是预编译模块，picker 的启动延迟更低
- **统一的 Git 集成**：Snacks 内置了 git_files、git_status、git_log、git_diff、git_branches、git_blame 等完整的 Git picker
- **自由的扩展能力**：通过 `Snacks.picker()` API 可以轻松创建自定义 picker
