...
这套配置的 Git 仓库始于 **2017 年 12 月 13 日**，至今已有 2011 次提交，跨越了八年多时间。它的演进过程本身就是终端编辑器生态变迁的一个缩影：
...
如今 Neovim 0.10+ 的 Lua 生态已经足够成熟，venux 在保持近 60 个插件的同时，启动时间稳定在 **100ms 以内**。
...
这些优化使得 venux 在加载近 60 个插件的情况下，启动时间仍然控制在 100ms 以内（dashboard 会显示精确的插件加载数量和时间）。
...
-- 自动格式化（保存时触发）
-- Rust 和 Lua 保存时自动格式化
-- Go 保存时额外执行 organize imports + formatting
```
...
Telescope 是 Neovim 生态中最成熟的模糊搜索框架之一。venux 对其做了深度定制：
...
2. **深度的 LSP 集成**：近 20 个语言服务器配置，每个都经过调校
